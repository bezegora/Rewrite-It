...
namespace Rewrite_It
{
    public class CheckMode
    {
        ...
        public CheckMode(Controller controller)
        {
            ...
            AddMistake(MistakeType.PseudoLanguage);
        ...
        public (string name, string description) GetMistakeText(MistakeType mistake)
        {
            switch (mistake)
            {
                case MistakeType.NoNumbers: return
                        ("Голые числительные",
                            "Все числительные следует писать цифрами (\"80\" вместо \"восемьдесят\")" +
                            "Человеческий глаз очень цепок на подобные \"крючки\"" +
                            "Числа в тексте добавляют ему убедительности");
                ...
                case MistakeType.PseudoLanguage: return 
                        ("Псевдоязык целевой аудитории",
                            "Эту ошибку можно обозначит как \"попытка сойти за своего\" или \"попытка в сленг\"" +
                            "\n\"быть своим\" - гиблая стратегия, люди чувствуют если вы пытаетесь говорить" +
